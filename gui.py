...
class CompiladorGUI:
    ...
    def criar_interface(self):
        ...
        self.text_editor.tag_config('error_line', background='#fee2e2', foreground='#991b1b')
    ...
    def limpar_destaques_erro(self):
        """Remove todos os destaques de erro do editor"""
        self.text_editor.tag_remove('error_line', '1.0', tk.END)
    
    def destacar_linha_erro(self, numero_linha):
        """Destaca uma linha específica com cor de erro"""
        try:

            self.limpar_destaques_erro()
            
            inicio = f"{numero_linha}.0"
            fim = f"{numero_linha}.end"
            self.text_editor.tag_add('error_line', inicio, fim)
            

            self.text_editor.see(inicio)
            

            self.piscar_linha_erro(numero_linha, 3)
            
        except Exception as e:
            print(f"Erro ao destacar linha: {e}")
    
    def piscar_linha_erro(self, numero_linha, vezes):
        """Faz a linha piscar para chamar atenção"""
        def piscar(contador):
            if contador > 0:
                if contador % 2 == 0:
                    self.text_editor.tag_config('error_line', background='#fecaca', foreground='#991b1b')
                else:
                    self.text_editor.tag_config('error_line', background='#fee2e2', foreground='#991b1b')
                self.root.after(200, lambda: piscar(contador - 1))
            else:
    ...
    def extrair_numero_linha_erro(self, texto_erro):
        """Extrai o número da linha do texto de erro"""

        match = re.search(r'linha\s+(\d+)', texto_erro, re.IGNORECASE)
        if match:
    ...
    def executar_compilacao(self):
        try:
            ...
            if "sucesso" in output_completo.lower() or result.returncode == 0:
                ...
                numero_linha = self.extrair_numero_linha_erro(output_completo)
                if numero_linha:
                    self.root.after(100, lambda: self.destacar_linha_erro(numero_linha))
